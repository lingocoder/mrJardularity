...
public final class CompileModuleInfoHelper {
    ...
    public static void dependOnOtherCompileModuleInfoJavaTasks(JavaCompile javaCompile) {
        dependentCompileModuleInfoJavaTaskStream(javaCompile.getProject())
                .peek( (@Nonnull var compileModuleInfoJava) -> LOGGER.debug("{}.dependsOn({})", javaCompile, compileModuleInfoJava))
    ...
    private static Stream<Task> dependentCompileModuleInfoJavaTaskStream(Project project) {
        return project.getConfigurations().stream()
                .flatMap( ( @Nonnull var configuration ) -> configuration.getDependencies().stream())
                .filter( ( @Nonnull var dependency ) -> dependency instanceof ProjectDependency)
                .map(( @Nonnull var dependency ) -> ((ProjectDependency) dependency).getDependencyProject().getTasks())
                .map( ( @Nonnull var tasks ) -> tasks.findByName(CompileModuleOptions.COMPILE_MODULE_INFO_TASK_NAME))
